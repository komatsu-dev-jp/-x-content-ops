...
`.claude/skills/x-account-launch/` はアカウントの初期設計（レーン選定・表示名・bio・アイコン方針・
固定ポスト・最初の30日プラン）と既存プロフィールの診断を行うSkillです。日々の投稿づくり
（`x-draft-post` 等）とは別のフェーズを担当し、根拠は `references/` の調査ダイジェストに置いています。
アカウント作成・プロフィール変更・投稿は行わず、案の提示までで止まります。
